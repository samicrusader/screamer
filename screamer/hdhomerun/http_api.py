...
def lineup():
    ...
    for _, channel in current_app.app_config['channels'].items():
        ...
        ElementTree.SubElement(program, 'GuideNumber').text = str(channel['master_channel'])+'.'+str(channel['virtual_channel'])
        ...
        ElementTree.SubElement(program, 'URL').text = f'http://127.0.0.1:5004/auto/v{str(channel["master_channel"])+"."+str(channel["virtual_channel"])}'
